...
// TestProcessIndividualSpanAttribute verifies the Notion fork's partial trace
// sampling routing: an event carrying meta.refinery.individual_span must be
// handed to the collector for an immediate independent decision instead of
// normal trace collection, peer forwarding, or direct transmission.
func TestProcessIndividualSpanAttribute(t *testing.T) {
	...
	require.NotNil(t, mockCollector.ProcessedIndividualSpan, "Expected a span to be processed individually")
	assert.Equal(t, "test-trace-123", mockCollector.ProcessedIndividualSpan.TraceID)
	assert.Equal(t, true, mockCollector.ProcessedIndividualSpan.Data.Get("meta.refinery.individual_span"))
	assert.Equal(t, "test_value", mockCollector.ProcessedIndividualSpan.Data.Get("test_attribute"))
